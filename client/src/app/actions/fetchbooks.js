 ...
 */
export const fetchAllBooks = () => dispatch => (api.book.fetchAllBooks().then((response) => {
  dispatch(fetchBooksfufilled(response.data));
}).catch((error) => {
  dispatch(fetchBooksRejected(error.response.data.message));
}));
