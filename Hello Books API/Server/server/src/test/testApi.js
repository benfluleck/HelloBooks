...
describe('HelloBooks', () => {
  ...
  describe('POST /login', () => {
    it('it responds with 401 status code if bad username or password', (done) => {
      chai.request(app).post('api/users/signin').send({ username: faker.internet.userName(), password: faker.internet.password }).end((err, res) => {
        expect(403);
        done();
      });
    });
    ...
    it('it responds with 202 status code if good username or password', (done) => {
      chai.request(app).post('/api/users/signin').send({ username: 'Benny', password: 'benny' }).end((err, res) => {
    ...
    it('it allows the user to loan a book', (done) => {
      ...
      chai.request(app).post(`/api/users/${  userbook.userid  }/books`).set('x-access-token', token).send(userbook).end((err, res) => {
    ...
    describe('/GET', () => {
      it('It retrieves all books from the data', (done) => {
        chai.request(app).get('/api/books').set('x-access-token', token).end((err, res) => {
          // bookid = Books.id
          expect(res.status).to.equal(200);
          done();
        });
