...
describe('Authentication', () => {
  it('should return 401 if token is not a token', (done) => {
    chai.request(app).put('/api/v1/users/changepassword')
      .set('x-access-token', '')
      .send({
        newPassword: 'bennyogidan23'
      })
      .end((err, res) => {
  ...
  it('should return 403 if token is not an admin token', (done) => {
    chai.request(app).post('/api/v1/admin/category').set('x-access-token', token)
      .send({
  ...
  it('should return 401 if error with token', (done) => {
    chai.request(app).post('/api/v1/admin/category').set('x-access-token', errorToken)
      .send({
        categoryName: 'EDUCATIONAL'
      })
      .end((err, res) => {
